...
N_swd_interpolatedLBA <- na.approx(SWD_dataLBA$N_swd, na.rm = TRUE, rule=2,maxgap=60)
date <- SWD_dataLBA$YYYYMMDD

swd_interpolated2LBA <-data.frame(N_swd_interpolatedLBA = N_swd_interpolatedLBA, YYYYMMDD = date)
colnames(swd_interpolated2LBA)[colnames(swd_interpolated2LBA) == "N_swd_interpolatedLBA"] <- "N_swd"
swd_interpolated2LBA<-na.omit(swd_interpolated2LBA)
